...
__global__ void cal_pi(Real *sum, int nbin, 
    Real step, int nthreads, int nblocks) {
	int i;
	Real x;
...
int main(void) {
    ...
    printf("# of trials per thread = %d, # of blocks = %d,\n # of threads/block = %d\n",
    TRIALS_PER_THREAD,
    NUM_BLOCK,
    NUM_THREAD);

	Real step = 1.0/NBIN;  // Step size
    size_t size = NUM_BLOCK*NUM_THREAD*sizeof(Real);  
    //Array memory size
    ...
    cal_pi <<<dimGrid, dimBlock>>> (sumDev, NBIN, 
        step, NUM_THREAD, NUM_BLOCK); // call CUDA kernel
